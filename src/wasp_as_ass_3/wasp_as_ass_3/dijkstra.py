...
import heapq as hq
from wasp_as_ass_3.grid_map import GridMap
from wasp_as_ass_3.utility import reconstruct_path, cell_to_coord_path
...
def dijkstra(gm: GridMap, x1: float, y1: float, x2: float, y2: float, eight_connectivity: bool) -> tuple[list[tuple[float, float]], set[tuple[int, int]]]:
    start = gm.cell(x1, y1)
    goal = gm.cell(x2, y2)
    ...
    distance = {start: 0.0}
    visited = set()
    to_be_visited = [(distance[start], 0, start)]
    count = 1
    ...
    while to_be_visited:
        ...
        for neighbor in neighbors(gm, *current, eight_connectivity):
            ...
            if neighbor_dist < distance.get(neighbor, float('inf')):
                ...
                hq.heappush(to_be_visited, (neighbor_dist, -count, neighbor))
                count += 1

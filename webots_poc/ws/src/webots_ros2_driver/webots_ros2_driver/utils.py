...
class WebotsVersion:
    ...
    def from_path(path):
        if sys.platform == 'darwin':
            # EXPERIMENTAL macOS fix: version.txt lives inside the .app bundle's
            # Contents/Resources, not directly under a flat "resources" dir like
            # on Linux/Windows.
            version_file = os.path.join(path, 'Contents', 'Resources', 'version.txt')
        else:

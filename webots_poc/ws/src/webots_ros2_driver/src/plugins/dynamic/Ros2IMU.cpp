...
namespace webots_ros2_driver {
  void Ros2IMU::init(webots_ros2_driver::WebotsNode *node, std::unordered_map<std::string, std::string> &parameters) {
    ...
    mRpyPublisher =
      mNode->create_publisher<geometry_msgs::msg::Vector3Stamped>(mTopicName + "/rpy", rclcpp::SensorDataQoS().reliable());
    mRpyMessage.header.frame_id = mFrameName;
  ...
  void Ros2IMU::publishData() {
    ...
    if (mInertialUnit) {
      ...
      mRpyMessage.header.stamp = mMessage.header.stamp;
      mRpyMessage.vector.x = rpy[0];
      mRpyMessage.vector.y = rpy[1];
      mRpyMessage.vector.z = rpy[2];
      mRpyPublisher->publish(mRpyMessage);
    }
